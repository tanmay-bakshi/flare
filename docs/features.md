...
| `WsClient.split(max_message_bytes)` / `WsConnection.split(max_message_bytes)` — role-correct full-duplex sender/receiver ownership with an explicit complete-message cap, independent idempotent shutdown, and serialized application and control writes | [`tests/ws/test_ws_duplex_wss.mojo`](../tests/ws/test_ws_duplex_wss.mojo), [`tests/ws/test_ws_duplex_bounds.mojo`](../tests/ws/test_ws_duplex_bounds.mojo), `flare.ws._duplex` |
...
| `WsPublicationAction` — opaque bounded work invoked by the I/O owner at final-byte publication, before sender completion becomes observable; destruction settles every non-publication path | [`tests/ws/test_ws_publication_action.mojo`](../tests/ws/test_ws_publication_action.mojo) |
| `WsServer` — server-side handshake + frame loop | [`ws_server.mojo`](../examples/basic/ws_server.mojo) |
| `WsHandler` trait + `WsServer.serve[H]` — stateful struct handler that owns each accepted `WsConnection`, allowing linear split while retaining mutable handler state | [`tests/ws/test_ws_stateful_handler.mojo`](../tests/ws/test_ws_stateful_handler.mojo) |
